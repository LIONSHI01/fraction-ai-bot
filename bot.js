...
fractionai 自动化机器人 v2.1.0`;
  ...
  console.log(
    chalk.bgHex("#1A1A1A").hex("#00FF88")(
      " 加入我们：电报频道：https://t.me/ksqxszq "
    ) +
      输出样式.颜色主题.数值("  v2.1.0") +
      "\n" +
...
function (剩余时间, 总时间) {
  ...
  return 输出样式.颜色主题.强调(
    "⏳ " +
      chalk.hex("#FFAA33")(
        `[${"█".repeat(Math.round(25 * (1 - 进度百分比)))}${"░".repeat(
          25 - Math.round(25 * (1 - 进度百分比))
        )}] `
      ) +
...
async function 加载钱包文件() {
  try {
      ...
      .filter((私钥) => {
        ...
        if (!有效格式 && 系统配置.调试模式) {
          console.log(
            输出样式.状态图标.警告 +
              " " +
              chalk.yellow(`无效私钥：${私钥.slice(0, 8)}...`)
          );
        }
        return 有效格式;
      });
    ...
    console.log(
      输出样式.状态图标.错误 + " " + chalk.red("钱包文件加载失败："),
      错误.message
    );
    process.exit(1);
  }
...
class 区块链认证器 {
  ...
  async 执行认证流程(私钥) {
    try {
      if (系统配置.调试模式) {
        console.log(
          输出样式.颜色主题.次要(`[调试] 原始私钥: ${私钥.slice(0, 8)}...`)
        );
      }
      ...
      if (时间偏差 > 系统配置.运行控制.最大时间偏差) {
        console.log(
          输出样式.状态图标.错误 + " " + chalk.red("系统时间不同步！偏差："),
      ...
      const 随机数响应 = await axios.get(
        `${系统配置.网络设置.API地址}/api3/auth/nonce`
      );
...
Issued At: ${签发时间}`;
      ...
      console.log(
        输出样式.状态图标.成功 +
          ...
          ` ${输出样式.颜色主题.地址(
            钱包账户.address.slice(0, 6) + "..." + 钱包账户.address.slice(-4)
          )}` +
          输出样式.颜色主题.次要(` · 积分: `) +
...
class 代理管理器 {
  static async 获取代理信息(访问令牌, 用户ID) {
    try {
      ...
      console.log(
        输出样式.状态图标.警告 + " " + chalk.yellow("代理信息获取失败："),
    ...
    代理列表.forEach((代理, 序号) => {
      ...
      console.log(
        ` ${chalk.gray("┝")} ${chalk.bold(`#${序号 + 1}`)} ` +
          ...
          `${状态颜色(`[${代理.运行状态}]`)} ` +
          输出样式.颜色主题.时间(
            `创建于 ${代理.创建时间.toLocaleDateString("zh-CN")}`
          )
      );
    });
...
async function 处理单个钱包(私钥, 序号, 总数) {
  ...
  const 表格数据 = [
    {
      ...
      地址: 输出样式.颜色主题.地址(
        `${认证结果.钱包地址.slice(0, 6)}...${认证结果.钱包地址.slice(-4)}`
      ),
      代理数:
        代理列表.length > 0 ? chalk.green(代理列表.length) : chalk.red("×"),
      积分: 输出样式.颜色主题.数值(`✦${认证结果.用户信息.fractal}`),
      状态:
        代理列表.length > 0
          ? chalk.bgGreen.black(" ✅ ONLINE ")
          : chalk.bgRed.black(" ❌ OFFLINE "),
    },
  ];
  ...
  表格数据.forEach((row) => {
    表格.push([row.序号, row.地址, row.代理数, row.积分, row.状态]);
  });
    ...
    while (true) {
      ...
      const 倒计时间隔 = setInterval(() => {
        ...
        logUpdate(
          `\n${生成倒计时条(剩余时间, 系统配置.运行控制.循环间隔)}\n` +
            gradient.passion(
              `  下一轮循环将在 ${Math.ceil(剩余时间 / 1000)} 秒后开始...`
            )
        );
      ...
      console.log(
        `\n${输出样式.状态图标.进度} ` +
          输出样式.颜色主题.强调(`下次循环于 `) +
          输出样式.颜色主题.时间(
            `${(系统配置.运行控制.循环间隔 / 60000).toFixed(2)} 分钟`
          ) +
          输出样式.颜色主题.强调(` 后开始 `) +
...
主流程().catch((error) => {
  console.error(
    chalk.bgRed.white.bold(` ‼ 未处理的系统错误: ${error.message} `)
  );
  process.exit(1);
});
